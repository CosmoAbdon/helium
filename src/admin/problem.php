	...
	if(isset($_POST['basename']) &&
	   ...
	   $_POST["confirmation"] == "confirm") {
		...
		if(@mkdir($tfile . "_d", 0700)) {
			...
			$filea = array('compare' . $ds . 'c','compare' . $ds . 'cpp','compare' . $ds . 'java', 'compare' . $ds . 'py',
						   'compile' . $ds . 'c','compile' . $ds . 'cpp','compile' . $ds . 'java', 'compile' . $ds . 'py',
						   'run' . $ds . 'c','run' . $ds . 'cpp','run' . $ds . 'java', 'run' . $ds . 'py');
			...
			file_put_contents($dir . $ds . 'limits' . $ds . 'py',$str);
